 ...
 * 
 * این دستور برای انتقال ایمن schema از دیتابیس A به B طراحی شده
 * با قابلیت‌های:
 * - مقایسه دقیق schema دو دیتابیس
 * - Smart-skip: خودکار skip کردن migrationهای تکراری
 * - محافظت از جداول محافظت شده (users, settings)
...
class DbDiffCommand extends Command
{
    ...
    /**
     * Migrationهای محافظت شده که فقط به عنوان Ran مارک می‌شوند
     * این migrationها معمولاً برای جداول حیاتی مثل users و settings هستند
     */
    ...
    public function handle(): int
    {
        ...
        if ($this->option('details')) {
            $this->line(PHP_EOL.'=== Detailed column diffs ===');
            foreach ($columnDiffs as $t => $d) {
                if (empty($d['changed'])) continue;
                $this->comment("🧩 {$t}");
                $rows=[];
                foreach ($d['changed'] as $c) {
                    $rows[] = [$c, json_encode($d['colsA'][$c]), json_encode($d['colsB'][$c])];
                }
                $this->table(['Column', 'A', 'B'], $rows);
            }
        }
        ...
        if ($this->option('apply')) {
            ...
            $bConn = $this->option('b-connection') ?: 'mysql_b';
