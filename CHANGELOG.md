...
### ⚙️ Added
- **Image Uploader Assign Toggle**: گزینه‌ی جدید `enableAssignButton` برای کنترل نمایش دکمه «اتصال به ترکیب» و پشتیبانی از تنظیم شناسه‌ی سلکتور (`combinationSelectorId`) اضافه شد تا پروژه‌ها بتوانند این قابلیت را بر اساس نیازشان فعال یا غیرفعال کنند.

### 🐛 Fixed
- **Image Uploader Actions Layout**: چینش دکمه‌های اکشن (کاور، اتصال، حذف) در پیش‌نمایش چند فایل بازطراحی شد (`d-flex + gap`) تا ارتفاع یکسان و فاصله‌گذاری منظم داشته باشند و دکمه‌ها زیر هم نیفتند.
